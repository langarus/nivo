...
export declare const computeShapeGenerators: <D extends FunnelDatum<Record<string, string | number>>>(interpolation: "smooth" | "linear", direction: import("./types").FunnelDirection) => [FunnelAreaGenerator, FunnelBorderGenerator];
interface CustomBandScale {
    (index: number): number;
    bandwidth: number;
}
export declare const computeScales: <D extends FunnelDatum<Record<string, string | number>>>({ data, direction, width, height, spacing, }: {
    data: D[];
...
export declare const computeSeparators: <D extends FunnelDatum<Record<string, string | number>>>({ parts, direction, width, height, spacing, enableBeforeSeparators, beforeSeparatorOffset, enableAfterSeparators, afterSeparatorOffset, }: {
    parts: FunnelPart<D>[];
    direction: import("./types").FunnelDirection;
    width: number;
    height: number;
...
export declare const computePartsHandlers: <D extends FunnelDatum<Record<string, string | number>>>({ parts, setCurrentPartId, isInteractive, onMouseEnter, onMouseLeave, onMouseMove, onClick, showTooltipFromEvent, hideTooltip, tooltip, }: {
    parts: FunnelPart<D>[];
...
export declare const getSizeGenerator: <Datum extends FunnelDatum<Record<string, string | number>>>(config: SizeSpec<Datum> | undefined, linearScale: ScaleLinear<number, number>) => (datum: Datum) => number;
export declare const useSize: <Datum extends FunnelDatum<Record<string, string | number>>>(config: SizeSpec<Datum> | undefined, linearScale: ScaleLinear<number, number>) => (datum: Datum) => number;
/**
 ...
 */
export declare const useFunnel: <D extends FunnelDatum<Record<string, string | number>>>({ data, width, height, direction, interpolation, spacing, shapeBlending: rawShapeBlending, valueFormat, colors, size, fillOpacity, borderWidth, borderColor, borderOpacity, labelColor, enableBeforeSeparators, beforeSeparatorLength, beforeSeparatorOffset, enableAfterSeparators, afterSeparatorLength, afterSeparatorOffset, isInteractive, currentPartSizeExtension, currentBorderWidth, onMouseEnter, onMouseMove, onMouseLeave, onClick, tooltip, }: {
    data: D[];
    width: number;
    height: number;
...
export declare const useFunnelAnnotations: <D extends FunnelDatum<Record<string, string | number>>>(parts: FunnelPart<D>[], annotations: import("@nivo/annotations").AnnotationMatcher<FunnelPart<D>>[]) => import("@nivo/annotations").BoundAnnotation<FunnelPart<D>>[];
export {};
