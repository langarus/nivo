...
export declare const useDataDimensions: <RawDatum>(rawDimensions: readonly import("./types").RawDimensionDatum<RawDatum>[]) => {
    dimensionIds: string[];
    dimensions: Record<string, (datum: RawDatum) => number>;
};
export declare const useStack: <RawDatum>(dimensionIds: readonly string[], dimensions: Record<string, (datum: RawDatum) => number>, offset: OffsetId) => Stack<any, RawDatum, string>;
export declare const useStackedData: <RawDatum>(stack: Stack<any, RawDatum, string>, data: readonly RawDatum[]) => {
...
export declare const useNormalizedData: <RawDatum>(data: readonly RawDatum[], id: string | number | DatumPropertyAccessor<RawDatum, import("./types").DatumId>, value: string | number | DatumPropertyAccessor<RawDatum, number>) => NormalizedDatum<RawDatum>[];
...
export declare const useComputedData: <RawDatum>({ data, stacked, rawDimensions, valueFormat, thicknessScale, dimensionsScale, colors, layout, outerPadding, innerPadding, }: {
    data: NormalizedDatum<RawDatum>[];
    stacked: Series<RawDatum, string>[];
    rawDimensions: readonly import("./types").RawDimensionDatum<RawDatum>[];
...
export declare const useMarimekko: <RawDatum>({ data, id, value, valueFormat, dimensions: rawDimensions, layout, offset, outerPadding, innerPadding, colors, borderColor, borderWidth, width, height, }: {
    ...
    valueFormat: import("@nivo/core").ValueFormat<number, void> | undefined;
    dimensions: readonly import("./types").RawDimensionDatum<RawDatum>[];
    layout: Layout;
    offset: OffsetId;
    outerPadding: number;
    innerPadding: number;
